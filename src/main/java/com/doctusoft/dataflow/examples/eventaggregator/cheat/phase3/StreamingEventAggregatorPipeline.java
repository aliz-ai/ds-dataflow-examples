...
public class StreamingEventAggregatorPipeline {
    ...
    public static void main(String[] args) {
        ...
        pipeline.getCoderRegistry().registerCoderForClass(JsonParsingError.class, SerializableCoder.of(JsonParsingError.class));
        ...
        TableReference jsonParsingErrorTable = new TableReference();
        jsonParsingErrorTable.setProjectId(options.getProject());
        jsonParsingErrorTable.setDatasetId(options.getBigqueryDatasetId());
        jsonParsingErrorTable.setTableId(JsonParsingError.class.getSimpleName().toLowerCase());
        
        TupleTag<JsonParsingError> jsonParsingErrorTupleTag = new TupleTag<JsonParsingError>() {};
        TupleTag<Event> jsonParsingResultTupleTag = new TupleTag<Event>() {};
        
        PCollectionTuple jsonParsingResultTuple = pipeline.apply(PubsubIO.readStrings()
                                                                         .fromTopic(String.format("projects/%s/topics/%s", options.getProject(), options.getSourceTopic())))
                                                          .apply(ParDo.of(JsonParserDoFn.of(Event.class, jsonParsingErrorTupleTag))
                                                                      .withOutputTags(jsonParsingResultTupleTag, TupleTagList.of(jsonParsingErrorTupleTag)));
        
        jsonParsingResultTuple.get(jsonParsingErrorTupleTag).apply(BigQueryIO.<JsonParsingError>write()
                                                                           .to(jsonParsingErrorTable)
                                                                           .withCreateDisposition(BigQueryIO.Write.CreateDisposition.CREATE_IF_NEEDED)
                                                                           .withWriteDisposition(BigQueryIO.Write.WriteDisposition.WRITE_APPEND)
                                                                           .withFormatFunction(JsonParsingError.formatFunction())
                                                                           .withSchema(JsonParsingError.bqSchema()));
        
        PCollection<Event> events = jsonParsingResultTuple.get(jsonParsingResultTupleTag)
                                                          ...
                                                          .apply(WithTimestamps.of((Event e) -> e.getTimestamp().toInstant())
